...
const pathConfig = JSON.parse(fs.readFileSync('./path.json', 'utf-8'));
...
ipcMain.on(channel.SENDING_OUTPUT_PARAMS, (event, arg) => {
  fs.writeFileSync(`.${pathConfig.predictConfig}`, arg, "utf-8");
  childProcess.exec(pathConfig.runPredict, (err, stdout, stderr) => {
    event.reply(channel.SENDING_OUTPUT_PARAMS, stdout);
    shell.openPath(`C:\\Users\\USER\\Desktop\\prediction GUI\\pack${pathConfig.outPath}`);
  })
});
